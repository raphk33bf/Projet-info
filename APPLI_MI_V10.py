...
#--------------1-Determiner les moyens de transports utilisés sur une période de la journée----------------
 ...
 #--------------2-Demarrer un trajet et determiner le moyen de transport utilisé----------------       
    ...
    def un_debut_gps(self, instance, **kwargs):
        ...
        req_method = 'POST'
        UrlRequest('http://irioso.free.fr/Groupe_1/lieu_d_interet_local.php', req_body=req_body, method=req_method, on_success=self.affiche_lieu)
    ...
    def un_envoi_moy(self, instance):
        ...
        UrlRequest(url='http://irioso.free.fr/Ambrasobin/insert_trajet.php', req_body=json.dumps(data),  req_headers=headers, on_success=self.data_consolidees)
 ...
 #------------3-Bonus - Demarrer un trajet et determiner les moyens de transport utilisés----------------          
    ...
    def recup_donnees_gps(self, **kwargs):
        ...
        UrlRequest(url='http://irioso.free.fr/Ambrasobin/recup_donnees.php', req_body=json.dumps(data),  req_headers=headers, on_success=self.data_consolidees)
    ...
    def data_consolidees(self, requete, resultat):
        data = {'id_phone': self.id_phone, 'long': self.longitude, 'timestamp': self.timestamp; 'lat' : self.latitude, 'speed': self.speed, 'acc': self.accuracy, 'bearing': self.bearing, 'time': self.timestamp_debut, 'alt': self.altitude}
